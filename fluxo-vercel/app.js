  ...
  <div class="app">
    <aside class="side" id="side">
      ...
      <nav class="nav">
        ...
        ${navBtn('reservas','◫','Reserva quinzenal')}
...
<section id="contas" class="section"><div class="top"><div class="title"><h1>Contas fixas</h1><p>Compromissos recorrentes com vencimento flexível.</p></div><button class="primary" onclick="openFixedModal()">+ Nova conta</button></div><div class="notice">Você pode usar dia do mês, 1º/5º/15º dia útil ou último dia útil.</div><div class="card table-card" id="fixedTable"></div></section>`}

function reserveSection(){return `
<section id="reservas" class="section"><div class="top"><div class="title"><h1>Reserva quinzenal</h1><p>Separe parte das contas no primeiro pagamento e complete no fim do mês.</p></div><button class="primary" onclick="openModal('mReserve')">+ Criar reserva</button></div><div class="notice">Exemplo: uma conta de R$ 100 pode ser separada em R$ 50 no primeiro recebimento e R$ 50 no segundo.</div><div id="reserveList" class="grid card-grid"></div></section>`}
...
function renderAll(){
  ...
  renderBars(monthTx); renderUpcoming(); renderRecent(monthTx); renderCalendar(); renderReserveSummary(); renderFutureInvoices(); renderTables(); renderReserveList(); renderCards(); renderPlanned(); renderVA(vu,used,bal,pct); renderGoals();
}
...
function renderTables(){
  ...
  fixedTable.innerHTML=data.fixed.length?`<div class="table-wrap"><table class="table"><tr><th>Conta</th><th>Quando vence</th><th>Valor</th><th>Ações</th></tr>${data.fixed.map(x=>`<tr><td data-label="Conta"><b>${esc(x.name)}</b>${x.due_month?`<div class="sub">Somente em ${MONTHS[x.due_month-1]}</div>`:''}</td><td data-label="Vencimento">${dueLabel(x)}</td><td data-label="Valor">${fmtMoney(x.amount)}</td><td data-label="Ações"><div class="btn-row"><button class="ghost" onclick="editFixed(${x.id})">Editar</button><button class="danger-btn" onclick="delRow('fixed_bills',${x.id})">Excluir</button></div></td></tr>`).join('')}</table></div>`:'<div class="empty">Nenhuma conta fixa.</div>';
}

function renderReserveSummary(){
  if(!data.reservePlans.length){reserveSummary.innerHTML='<div class="empty">Crie reservas para suas contas.</div>';return;}
  const key=firstOfMonth(cur);
  reserveSummary.innerHTML=data.reservePlans.slice(0,5).map(p=>{
    const e=data.reserveEntries.find(x=>x.plan_id===p.id&&x.reference_month===key); const saved=Number(e?.first_saved||0)+Number(e?.second_saved||0), pct=clamp(saved/Number(p.monthly_amount)*100,0,100);
    return `<div class="mini-plan"><div><b>${esc(p.name)}</b><div class="sub">${fmtMoney(saved)} de ${fmtMoney(p.monthly_amount)}</div></div><div class="mini-progress"><span style="width:${pct}%"></span></div></div>`;
  }).join('');
}
...
window.openFixedModal=()=>{editingFixedId=null;fixedModalTitle.textContent='Nova conta fixa';fd.value='';fv.value='';fmonth.value='';frule.value='calendar_day';fday.value='';fbday.value='';toggleFixedRuleFields();openModal('mFixed')};
window.editFixed=id=>{const x=data.fixed.find(x=>x.id===id);if(!x)return;editingFixedId=id;fixedModalTitle.textContent='Editar conta fixa';fd.value=x.name;fv.value=x.amount;fmonth.value=x.due_month||'';frule.value=x.due_rule||'calendar_day';fday.value=x.due_day||'';fbday.value=x.business_day_number||'';toggleFixedRuleFields();openModal('mFixed')};
window.saveFixed=async()=>{if(!fd.value.trim()||!+fv.value)return alert('Preencha nome e valor.');if(frule.value==='calendar_day'&&!+fday.value)return alert('Informe o dia do mês.');if(frule.value==='business_day_number'&&!+fbday.value)return alert('Informe o número do dia útil.');const payload={user_id:user.id,name:fd.value.trim(),amount:+fv.value,due_month:fmonth.value?+fmonth.value:null,due_rule:frule.value,due_day:frule.value==='calendar_day'?+fday.value:null,business_day_number:frule.value==='business_day_number'?+fbday.value:null};const r=editingFixedId?await sb.from('fixed_bills').update(payload).eq('id',editingFixedId):await sb.from('fixed_bills').insert(payload);if(r.error)return alert(r.error.message);closeModal('mFixed');await refresh()};
